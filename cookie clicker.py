...
class ClickerState:
    ...
    def __init__(self):
        ...
        self._cps = 1.0
        self._item = None
        self._history = [(self._time, self._item, 0.0, self._total)]
    ...
    def __str__(self):
        ...
        """
        out = "Time: " + str(self._time) + " Current Cookies: " + str(self._cookies) + " CPS: " + str(self._cps) + " Total Cookies: " + str(self._total) + " History(length: " + str(len(self._history)) + "): "  + str(self._history)
        return out
    ...
    def buy_item(self, item_name, cost, additional_cps):
        ...
        if self._cookies >= cost :
            ...
            self._cookies -= cost
            self._history.append((self._time, self._item, cost, self._total))
...
def strategy_best(cookies, cps, time_left, build_info):
    ...
    for item in cbi.build_items():
        if cbi.get_cost(item) <= time_left*cps + cookies:
            temp.append(cbi.get_cps(item)/cbi.get_cost(item))
            best.append(item)
    if len(best)>0:
