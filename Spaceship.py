...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile = Sprite([self.pos[0] + (vector[0] * self.radius),self.pos[1] + (vector[1] * self.radius)], [self.vel[0] + (vector[0] * 8),self.vel[1] + (vector[1] * 8)], 0, 0, missile_image, missile_info, missile_sound)
...
def draw(canvas):
    global time, lives, score, game_over, my_ship, a_rock
    ...
    if game_over == False:
        ...
        if dist(a_missile.pos, a_rock.pos) <= a_missile.radius + a_rock.radius :
            score += 1
    ...
    if game_over == True :
        canvas.draw_text("Game Over!", [WIDTH*0.3, HEIGHT/2], 75, "Red")
        timer.stop()
           
# timer handler that spawns a rock    
def rock_spawner():
    global a_rock
    a_rock = Sprite([WIDTH * random.random(), HEIGHT* random.random()], [random.random() * 1.2,random.random() * 1.2], 0, (random.random() ) /5, asteroid_image, asteroid_info)
